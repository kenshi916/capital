...
export function normalizeRound(input,companies){
 requireValue(!input.kind||input.kind==='treasury-policy','Choose a supported proposal type.');
 ...
 if(input.kind==='treasury-policy'){
  const topic=input.topic;requireValue(['purchase','distribution','reserve'].includes(topic),'Choose a treasury topic.');
  requireValue(Array.isArray(input.candidates)&&input.candidates.length>=2&&input.candidates.length<=6,'Include between 2 and 6 voting options.');
  const candidates=input.candidates.map(item=>{
   requireValue(item&&typeof item.id==='string'&&/^option-[a-z0-9-]{1,48}$/.test(item.id),'Use a stable voting option ID.');
   const checkedAt=Number(item.checkedAt||0);requireValue(Number.isSafeInteger(checkedAt)&&checkedAt>=0&&checkedAt<=now()+60,'Review dates cannot be in the future.');
   return {id:item.id,name:clean(item.name,100),image:'/assets/capital-mark.svg',platform:'Capital treasury',security:'Advisory policy choice',terms:clean(item.terms||'',2000,true),source:item.source?url(item.source):'',documents:item.documents?url(item.documents):'',status:'policy',checkedAt,reviewed:item.reviewed===true};
  });
  requireValue(new Set(candidates.map(c=>c.id)).size===candidates.length,'A voting option can only appear once.');
  requireValue(new Set(candidates.map(c=>c.name.toLowerCase())).size===candidates.length,'Give each voting option a different name.');
  return {title,notes,target,chainId,token,rule,minimum,opensAt,closesAt,candidates,kind:'treasury-policy',topic,execution:'advisory'};
 }
...
export function checkPublication(p,time=now()){
 ...
 if(p.kind==='treasury-policy')requireValue(p.candidates.every(c=>c.reviewed&&c.checkedAt>=time-7*86400&&c.terms&&c.documents),'Review each voting option’s terms and supporting document within seven days before publishing.');
 else requireValue(p.candidates.every(c=>c.reviewed&&c.checkedAt>=time-7*86400&&['open','secondary'].includes(c.status)&&c.terms&&c.documents),'Review every candidate’s current terms and document source within seven days before publishing. Closed or unverified offerings cannot enter voting.');
}
...
export function voteTypedData(round,p,wallet,candidate,sequence,nonce,expires,origin){
 const policy=p.kind==='treasury-policy',subject=policy?'decision':'business';
 return {domain:{name:'Capital community voting',version:policy?'2':'1',chainId:p.chainId,salt:hash(origin)},types:{Ballot:[{name:'roundTitle',type:'string'},{name:subject,type:'string'},{name:'wallet',type:'address'},{name:'roundId',type:'string'},{name:'revision',type:'uint256'},{name:'proposalHash',type:'bytes32'},{name:'snapshotBlock',type:'uint256'},{name:'snapshotHash',type:'bytes32'},{name:'choice',type:'string'},{name:'sequence',type:'uint256'},{name:'nonce',type:'string'},{name:'expiresAt',type:'uint256'}]},primaryType:'Ballot',message:{roundTitle:p.title,[subject]:p.candidates.find(c=>c.id===candidate).name,wallet,roundId:round.id,revision:round.revision,proposalHash:round.proposal_hash,snapshotBlock:round.snapshot_number,snapshotHash:round.snapshot_hash,choice:candidate,sequence,nonce,expiresAt:expires}};
}
