...
function setView(view){if(!['home','launch','explore','treasury','holdings','docs'].includes(view))view='home';const changed=state.view!==view;state.view=view;document.body.dataset.view=view;if(changed)window.scrollTo({top:0,behavior:'auto'});document.querySelectorAll('.view').forEach(el=>el.hidden=el.id!==`view-${view}`);document.querySelectorAll('[data-view]').forEach(el=>{const active=el.dataset.view===view;el.classList.toggle('active',active);if(active)el.setAttribute('aria-current','page');else el.removeAttribute('aria-current');});document.title=`${view==='docs'?'Docs':view==='home'?'Home':view==='launch'?'Launch setup':view==='explore'?'Businesses':view==='treasury'?'Treasury':'My portfolio'} — Capital`;}
function navigate(view){if(!['home','launch','explore','treasury','holdings','docs'].includes(view))throw new Error('Unknown view');if(location.hash!==`#${view}`)location.hash=view;else setView(view);}
let toastTimer;
...
function routeFromHash(){const raw=location.hash.slice(1),hash=raw==='docs'||raw.startsWith('docs/')?'docs':raw;if(hash==='main'&&document.body.dataset.view)return;setView((hash==='main'?'':hash)||(new URL(location.href).searchParams.has('treasury')?'treasury':'home'));}
