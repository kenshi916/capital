...
async function refresh() {
  ...
  if (!sameContract(code, state.artifacts.MainstreetTreasury)) throw new Error('This address is not the Mainstreet test-treasury contract for this version. No token approvals are enabled.');
...
function bindActions() {
  ...
  document.addEventListener('click', event => safely(async () => {
    ...
    if (target.closest('#chain-test-document')) {
      ...
      download(`mainstreet-test-document-${id + 1}.json`, object); text('#chain-evidence-status', `Labeled test document prepared. SHA-256: ${state.evidence}`);
    }
