...
async function checkPons() {
  ...
  try {
    ...
    result.innerHTML = `<strong>${d.token ? 'Pons V2 registration and references matched' : 'Robinhood Chain connection checked'}</strong><dl>${rows.map(([key,value])=>`<div><dt>${esc(key)}</dt><dd>${value}</dd></div>`).join('')}</dl><p>${d.token ? 'Escrow balances include all launches using that recipient. They are not Mainstreet-only revenue or holder payouts. Unswept estimates are not settled cash. This check does not establish who controls the token or provide a contract audit.' : 'No Mainstreet token address supplied. This check does not launch a token.'}</p>`;
...
function initialize() {
  ...
  $('.launch-status-strip').insertAdjacentHTML('beforebegin', `<section class="launch-route" aria-labelledby="pons-route-title"><div><span class="route-label">SELECTED LAUNCH ROUTE</span><h2 id="pons-route-title">Robinhood Chain + pons</h2><p>Mainstreet’s launch token is planned for Robinhood Chain mainnet, chain 4663. Business investments and holder rights require their own documented structure.</p><div class="launch-route-flow"><span>pons creator fees</span>→<span>Mainstreet treasury</span>→<span>Investments & payouts</span></div><p>Planned flow · collection, purchases and real payouts are not active.</p><div class="launch-route-links"><a href="https://www.ponsfamily.com/" target="_blank" rel="noopener noreferrer">Open pons ↗</a><a href="${PONS.source}" target="_blank" rel="noopener noreferrer">Pons V2 documentation ↗</a><button class="text-button" id="pons-preset">Use this network in my draft</button></div></div><div><span class="route-label">READ YOUR LAUNCH</span><h2>Check the actual token.</h2><p>Pons V2 documentation currently describes restricted launch access. The check reads access for your connected wallet and, when supplied, your token’s recipient and fees.</p><div class="launch-route-links"><button class="button button-dark" id="pons-check">Check network & token</button><button class="text-button" data-launch-stage="token">Edit token details ↗</button></div><div id="pons-result" class="pons-result" role="status" aria-live="polite"></div></div></section>`);
